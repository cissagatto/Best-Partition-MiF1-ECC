...
###############################################################################
FolderJob = paste(FolderRoot, "/bpmi-ecc-jobs", sep = "")
if (dir.exists(FolderJob) == FALSE) {dir.create(FolderJob)}

FolderCF = paste(FolderRoot, "/bpmi-ecc-config-files", sep="")
...
while(f<=length(similarity)){
  ...
  while(i <= n) {
    ...
    # job name
    job_name = paste("e-", sim[f], "-b-", dataset$Name, sep = "")

    # directory name
    folder_name = paste("/scratch/", job_name, sep = "")

    # Confi File Name
    config_name = paste(FolderS2, "/", sim[f], "-e-bpmi-",
    ...
    write("echo =================================================",
          file = output.file, append = TRUE)
    write("echo SBATCH: RUNNING BEST PARTITION MACRO-F1 ECC",
          file = output.file, append = TRUE)
    write("echo =================================================",
          file = output.file, append = TRUE)
    ...
    str7 = paste("Rscript /home/u704616/Best-Partition-MiF1-ECC/R/microf1.R \"",
                 config_name, "\"", sep = "")
